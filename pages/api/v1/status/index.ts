...
async function getHandler(request: NextApiRequest, response: NextApiResponse<StatusResponse>) {
...
async function getDatabaseMaxConnections(): Promise<number> {
  const result = await database.query<{ max_connections: string }>('SHOW max_connections;');

  return parseInt(result.rows[0]!.max_connections);
}

async function getDatabaseOpenedConnections(): Promise<number> {
  const databaseOpenedConnectionsResult = await database.query<{ count: number }>({
...
async function getDatabaseVersion(): Promise<string> {
  const databaseVersionResult = await database.query<{ server_version: string }>('SHOW server_version;');
