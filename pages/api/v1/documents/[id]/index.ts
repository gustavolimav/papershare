...
async function getHandler(
  ...
  const foundDocument = await document.findOneById(id, request.user!.id);

  return response.status(200).json(foundDocument);
}
...
async function patchHandler(
  ...
  const updatedDocument = await document.updateById(
    id,
    request.user!.id,
    updateInput,
  );
...
async function deleteHandler(
  ...
  const { storage_key } = await document.deleteById(id, request.user!.id);
