...
- [x] Storage adapter interface (`infra/storage.ts`) — local filesystem; swappable for S3/R2
- [x] File type validation (PDF, DOCX, PPTX only)
- [x] File size limit (configurable via `MAX_FILE_SIZE_MB`, default 50 MB)

### API

- [x] `POST /api/v1/documents` — Upload document (multipart/form-data, pdf-parse page count)
- [x] `GET /api/v1/documents` — List authenticated user's documents (paginated, DESC)
- [x] `GET /api/v1/documents/[id]` — Get document metadata
- [x] `PATCH /api/v1/documents/[id]` — Update title/description
- [x] `DELETE /api/v1/documents/[id]` — Soft-delete + physical file cleanup

### Model

- [x] `models/document.ts` — CRUD + two-step ownership check (404 vs 403)
- [x] PDF page count extraction on upload (pdf-parse)

### Tests

- [x] Upload, list, get, update, delete flows (5 test files, ~25 scenarios)
- [x] Authorization: user cannot manage another user's documents
- [x] Invalid file type / size / missing file: returns 400
