...
- Depends on: US-28 (workspace_id on documents), US-31 (documents resolve their workspace consistently — this story reuses that resolution rather than re-deriving it), and Phase 8's existing AI infrastructure (`infra/ai.ts`, `models/user.ts#getAiApiKey`) which is unchanged by this story — only the _input_ to `getAiApiKey()` changes.
